import { useState, useEffect } from 'react';
import PageContainer from "../../components/Assets/PageContainer";
import { Dropdown, DropdownBody, DropdownHead } from "../../components/Assets/Dropdown";
import { useUserData } from "../../components/Context/GlobalUserContext";
import RiseBalancePopup from "../../components/Popups/RiseBalancePopup";

export default function configurator({specifications}) {

  const [userData] = useUserData();

  const [channel, setChannel] = useState(0)
  const [cpu, setCpu] = useState(0)
  const [ssd, setSsd] = useState(0)
  const [ram, setRam] = useState(0)
  const [os, setOs] = useState(specifications.os[0].versions[0])

  const channelTotal = channel * specifications.Channel.price
  const cpuTotal = cpu * specifications.CPU.price
  const ssdTotal = ssd * specifications.SSD.price
  const ramTotal = ram * specifications.Ram.price

  const total = channelTotal + cpuTotal + ssdTotal + ramTotal + os.price

  return (
    <PageContainer>
        <section className="products display-flex">
          <div className="for-user for-config display-flex direction-column">
            <h1 className="heading">Конфигуратор</h1>
              <div className="config contet-space-b">
              <div>
                <h4>Легимитный канал</h4>
                <h1 className="value">{channel} Мбит/с</h1>
                <input onChange={e => setChannel(e.target.value)} type="range" min={specifications.Channel.min} max={specifications.Channel.max} value={channel} className="range"/>                            
                <h4>CPU</h4>
                <h1 className="value">{cpu} ядер</h1>
                <input onChange={e => setCpu(e.target.value)} type="range" min={specifications.CPU.min} max={specifications.CPU.max} value={cpu} className="range"/>                            
                <h4>SSD</h4>
                <h1 className="value">{ssd} Гб</h1>
                <input onChange={e => setSsd(e.target.value)} type="range" min={specifications.SSD.min} max={specifications.SSD.max} value={ssd} className="range"/>
                <h4>ОЗУ</h4>
                <h1 className="value">{ram} Гб</h1>
                <input onChange={e => setRam(e.target.value)} type="range" min={specifications.Ram.min} max={specifications.Ram.max} value={ram} className="range"/>                           
              </div>
              <div>  
                <div className="for-OS">
                  {
                    specifications.os.map((system, sys_idx) => 
                      <div>
                        <h2 className="items-center display-flex"><img src={system.img}/>{system.name}</h2>
                        <div className="for-inputs direction-column">
                          {
                            system.versions.map((version, idx) => 
                              <label className="chek-config contet-space-b display-flex" onClick={() => setOs(version)}>
                                <input type="radio" name="select" defaultChecked={idx == 0 && sys_idx == 0} />
                                <span>{version.name}</span>
                                <div className="dot"></div>
                              </label>
                            )
                          }
                        </div>
                      </div>
                    )
                  }
                </div>
              </div>
            </div>
          </div>
          <div className="for-user for-config-price display-flex direction-column">
            <h2 className="heading">Конфигурация сервера</h2>
            <h4>Легимитный канал</h4>
            <div className="profile-menu-mini contet-space-b"><h2 className="value-price">{channel} Мбит/с</h2><h5>{channelTotal} ₽</h5></div>
            <h4>CPU</h4>
            <div className="profile-menu-mini contet-space-b"><h2 className="value-price">{cpu} ядер</h2><h5>{cpuTotal} ₽</h5></div>
            <h4>SSD</h4>
            <div className="profile-menu-mini contet-space-b"><h2 className="value-price">{ssd} Гб</h2><h5>{ssdTotal} ₽</h5></div>
            <h4>ОЗУ</h4>
            <div className="profile-menu-mini heading contet-space-b"><h2 className="value-price">{ram} Гб</h2><h5>{ramTotal} ₽</h5></div>
            <h4>ОС</h4>
            <div className="profile-menu-mini heading contet-space-b"><h2 className="value-price">{os.name}</h2><h5>{os.price} ₽</h5></div>
            <h2>Итого</h2>
            <h1>{total} ₽/мес </h1>
            <Dropdown>
              <DropdownHead>
                <button>Заказать сервер</button>
              </DropdownHead>
              <DropdownBody>
                <div className="raise-balance items-center display-flex direction-column" id="modal-0">
                  <div className="for-raise-balance direction-column">
                    <div className="close-popup display-flex contet-space-b">
                      <h1 style={{color: "white", marginRight: 10+"px"}}>{
                        total <= userData.user.balance ?
                          "Вы уверены, что хотите оформить заказ?"
                        : 
                          "На Вашем балансе не достаточно средств"
                      }</h1>
                      <img closeHandler={(currentState) => ({state: !currentState})} id="close-popup" src="/assets/img/close_popup.svg"/>
                    </div>
                    {
                      total <= userData.user.balance ?
                        <>
                          <h2>Итого</h2>
                          <h1>{total} ₽/мес </h1>
                          <button>Заказать сервер</button>
                        </>
                      :
                        <div className="not_enough">
                          <div className="direction-column">
                            <h2>Сумма к оплате</h2>
                            <h1>{total} ₽/мес </h1>
                          </div>
                          <div className="direction-column">
                            <h2>Не хватает</h2>
                            <h1>{ total - userData.user.balance } ₽</h1>
                          </div>
                          <RiseBalancePopup/>
                        </div>
                    }
                    
                  </div>
                </div>
              </DropdownBody>
            </Dropdown>
            
          </div>
        </section>
    </PageContainer>
  )
}

export async function getServerSideProps({params}) {
  const response = {
    id: params.id,
    Channel: {
      min: 4,
      max: 98,
      price: 10
    },
    CPU: {
      min: 3,
      max: 60,
      price: 5
    },
    SSD: {
      min: 10,
      max: 50,
      price: 2
    },
    Ram: {
      min: 4,
      max: 40,
      price: 8
    },
    os: [
      {
        name: "Centos",
        img: "data:image/png;base64,",
        versions: [
          {
            name: "Centos 6",
            price: 1
          },
          {
            name: "Centos 7",
            price: 2
          },
          {
            name: "Centos 8",
            price: 3
          },
        ]
      },
      {
        name: "Debian",
        img: "data:image/png;base64,",
        versions: [
          {
            name: "Debian 8",
            price: 11
          },
          {
            name: "Debian 9",
            price: 12
          },
          {
            name: "Debian 10",
            price: 13
          },
        ]
      },
      {
        name: "Ubuntu",
        img: "data:image/png;base64,",
        versions: [
          {
            name: "Ubuntu 16.04",
            price: 21
          },
          {
            name: "Ubuntu 18.04",
            price: 22
          },
          {
            name: "Ubuntu 20.04",
            price: 23
          },
        ]
      },
      {
        name: "Windows Server",
        img: "data:image/png;base64,",
        versions: [
          {
            name: "Windows Server 2012 R2",
            price: 31
          },
          {
            name: "Windows Server 2016",
            price: 32
          },
          {
            name: "Windows Server 2019",
            price: 33
          },
        ]
      }
    ]
  };
  const specifications = response;

  return {
    props: {specifications}, // will be passed to the page component as props
  }
}